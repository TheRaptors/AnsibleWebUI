...
def get_playbooklists():
    '''
    :return:  返回playbook的项目列表
    '''

    return [i  for i in os.listdir(PRJ_DIR) if os.path.isdir(PRJ_DIR+i) and not i.startswith("\.") ]


def pblist(request):
    context_dict = {}
    if request.method == 'GET':
        pblst = get_playbooklists()
        context_dict['pblist'] = pblst
        return render(request, 'ansible/playrun.html', context_dict)
    elif request.method == 'POST':
        st = request.POST['site'].strip()
        site = PRJ_DIR+st+"/"+st+'.yml'
        print site
